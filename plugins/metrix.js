...
function initMetrix(MetrixAnalytics) {
  ...
  MetrixAnalytics.prototype.initialize = function(options) {
    appInfo = {
      package:
        options.packageName || document.location.hostname
    ...
    if (
      typeof options.uniqueDeviceId === "string" ||
      options.uniqueDeviceId instanceof String
    ) {
  ...
  MetrixAnalytics.prototype.sendRevenue = function(
    customName,
    amount,
    currency,
    orderId
  ...
  metrixQueue.setLastDataSendTryTime = function() {
    localStorage.setItem(
      localStorageKeys.lastDataSendTryTime,
  ...
  metrixQueue.breakHeavyQueue = function() {
    ...
    function refineQueue(inputQueue) {
      ...
      eventPriorities.forEach(function(type) {
        ...
        while (
          i < inputQueue.length &&
          newQueue.length < metrixSettingAndMonitoring.localQueueCapacity
        ) {
  ...
  metrixQueue.shouldAttemptSending = function() {
    ...
    if (lastSendTime != null) {
      ...
      if (
        diff < metrixSettingAndMonitoring.queueUnloadInterval &&
        this.isQueueNotLargeEnoughToSend()
      ) {
        return false;
      }
      ...
      if (lastSendTryTime != null) {
        ...
        if (
          diffTry >
          3 * metrixSettingAndMonitoring.timeOut +
            metrixSettingAndMonitoring.queueUnloadInterval
        ) {
  ...
  metrixQueue.updateSendingQueue = function() {
    ...
    metrixQueue.setSendingQueue(
      storedQueue.slice(0, metrixSettingAndMonitoring.updateChunkNumber)
    );
  };
  ...
  function addToQueue(event) {
    ...
    if (MetrixAppId != null) {
      ...
      metrixLogger.info("Metrix, A new Event was added to main queue", {
        type: event.type
      });
  ...
  function initDataSending() {
    if (
      metrixQueue.getMainQueue() != null &&
      metrixQueue.shouldAttemptSending()
    ) {
  ...
  function createSendingParcel() {
    ...
    if (ie) {
      ...
      if (ie > 11) connectionInfo.browserName = "MS Edge";
      if (ie <= 11) connectionInfo.browserName = "MSIE";
    }

    if (connectionInfo != null) meta.connection = connectionInfo;
  ...
  function attemptDataSending(parcel) {
    ...
    http.setRequestHeader(
      ...
      requestHeaders.ContentTypeValue
    );
  ...
  metrixSession.updateLastVisitTime = function() {
    localStorage.setItem(
      localStorageKeys.lastVisitTime,
      Utils.getCurrentTime().toString()
    );
  };
  ...
  metrixSession.updateSessionDuration = function() {
    ...
    localStorage.setItem(
      localStorageKeys.sessionDuration,
  ...
  metrixSession.setSessionIdLastReadTime = function() {
    localStorage.setItem(
      localStorageKeys.sessionIdLastReadTime,
      Utils.getCurrentTime().toString()
    );
  };
  ...
  metrixSession.generateNewSessionIfExpired = function() {
    ...
    if (
      this.sessionIdHasBeenRead() &&
      this.getSessionId() != null &&
      metrixSession.referrerHasNotChanged()
    ) {
      let timeSinceLastEvent =
        Utils.getCurrentTime() - this.getSessionIdLastReadTime();
      if (
        timeSinceLastEvent < metrixSettingAndMonitoring.sessionExpirationTime
      ) {
        this.setSessionIdLastReadTime();
        return;
      }
  ...
  Env.getDocumentData = function() {
    return {
      ...
      referrer:
        document.referrer || Utils.parseUrl(document.referrer) || undefined,
      url: Env.getUrlData()
    };
  };
  ...
  Env.getLocaleData = function() {
    ...
    let results = new RegExp("([A-Z]+-[0-9]+) \\(([A-Z]+)\\)").exec(
      new Date().toString()
    );
  ...
  Utils.getFormattedCurrentTime = function() {
    ...
    return (
      ...
      "T" +
      current_datetime.getUTCHours() +
      ":" +
      current_datetime.getUTCMinutes() +
      ":" +
      current_datetime.getUTCSeconds() +
      "." +
      current_datetime.getUTCMilliseconds() +
      "Z"
    );
  };
